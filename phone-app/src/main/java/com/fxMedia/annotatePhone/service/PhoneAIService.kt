package com.fxMedia.annotatePhone.service
...
import com.fxMedia.annotatePhone.MainActivity
import com.fxMedia.annotatePhone.R
import com.fxMedia.annotatePhone.data.AiProvider
...
import com.fxMedia.annotatePhone.data.db.ConversationRepository
import com.fxMedia.annotatePhone.data.db.RecordingRepository
import com.fxMedia.annotatePhone.service.ai.AiServiceFactory
import com.fxMedia.annotatePhone.service.ai.AiServiceProvider
import com.fxMedia.annotatePhone.service.ai.GeminiLiveSession
import com.fxMedia.annotatePhone.service.cxr.CxrMobileManager
import com.fxMedia.annotatePhone.service.stt.SttProvider
import com.fxMedia.annotatePhone.service.stt.SttService
import com.fxMedia.annotatePhone.service.stt.SttServiceFactory
import com.fxMedia.annotatePhone.data.toSttCredentials
import com.fxMedia.annotatePhone.service.ServiceBridge.notifyApiKeyMissing
...
class PhoneAIService : Service() {
    ...
    //var onSTTReady: ((String) -> Unit)? = null
    ...
    private val KEY_AZURE_SPEECH_KEY = "9gc9wRV6NllAZLejnO9Ua79H9osunQaPl7e1syWfglQ1siZqgYHcJQQJ99AKACqBBLyXJ3w3AAAYACOGVFeJ"
    private val KEY_AZURE_SPEECH_REGION = "southeastasia"
    ...
    private fun initializeServices() {
        ...
        try {
            ...
            settingsRepository.updateSttProvider(SttProvider.AZURE_SPEECH)

            settingsRepository.saveSettings(settingsRepository.getSettings().copy(
                sttProvider = SttProvider.AZURE_SPEECH,
                azureSpeechKey = KEY_AZURE_SPEECH_KEY,
                azureSpeechRegion = KEY_AZURE_SPEECH_REGION
            ))
    ...
    private suspend fun processVoiceData(audioData: ByteArray) {
        try {
            ...
            PhoneViewModel.updateTranscript(transcript)
    ...
    private suspend fun processPhoneRecording(recordingId: String, filePath: String) {
        ...
        try {
            ...
            val transcriptResult = performSpeechRecognition(audioData, filePath, "en-us")
...
class TextToSpeechService(private val context: android.content.Context) {
    ...
    private val edgeTtsClient = com.fxMedia.annotatePhone.service.EdgeTtsClient()
    ...
    fun speak(text: String, onAudioChunk: (ByteArray) -> Unit) {
        ...
        when (settings.ttsProvider) {
            com.fxMedia.annotatePhone.data.TtsProvider.EDGE_TTS -> speakWithEdge(text, settings, onAudioChunk)
            com.fxMedia.annotatePhone.data.TtsProvider.SYSTEM_TTS -> speakWithSystemTts(text, settings)
            com.fxMedia.annotatePhone.data.TtsProvider.GOOGLE_TRANSLATE_TTS -> speakWithSystemTts(text, settings)
        }
    }

    // ── Edge TTS ─────────────────────────────────────────

    private fun speakWithEdge(text: String, settings: com.fxMedia.annotatePhone.data.ApiSettings, onAudioChunk: (ByteArray) -> Unit) {
        ttsScope.launch {
            try {
                // Resolve voice
                val voice = if (settings.ttsVoiceOverride.isNotBlank()) {
                    ...
                    com.fxMedia.annotatePhone.ui.detectEdgeVoice(text, locale)
                }

                // Format rate & pitch
                val rate = com.fxMedia.annotatePhone.ui.formatEdgeRate(settings.ttsSpeechRate)
                val pitch = com.fxMedia.annotatePhone.ui.formatEdgePitch(settings.ttsPitch)
    ...
    private fun speakWithSystemTts(text: String, settings: com.fxMedia.annotatePhone.data.ApiSettings?) {
